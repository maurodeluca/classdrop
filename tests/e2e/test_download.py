...
@pytest.mark.e2e
def test_download_file_succeeds(tmp_path):
...
@pytest.mark.e2e
def test_download_file_with_no_file_metadata(tmp_path):
    """E2E test: verifies that a file cannot be downloaded if the file metadata does not exist."""
    ...
    assert data["detail"] == "File not found in metadata"

    # Cleanup
    app.dependency_overrides.clear()

@pytest.mark.e2e
def test_download_file_with_no_file_on_disk(tmp_path):
    """E2E test: verifies that a file cannot be downloaded if the file does not exist on the disk."""
    ...
    data = response.json()
    assert "detail" in data
    assert data["detail"] == "File not found on disk"
